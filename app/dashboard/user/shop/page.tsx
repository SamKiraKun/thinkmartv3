...
export default function ShopPage() {
  ...
  const buildRequest = useCallback(
    ...
    [
      ...
      filters.minPrice,
      filters.maxPrice,
      filters.minCoinPrice,
      filters.maxCoinPrice,
      filters.inStockOnly,
      filters.sort,
    ]
  );
  ...
  const loadFirstPage = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetchProducts(buildRequest(), 1, PAGE_SIZE);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchProducts(buildRequest(), nextPage, PAGE_SIZE);
      setProducts((prev) => {
        const map = new Map(prev.map((item) => [item.id, item]));
        response.data.forEach((item) => map.set(item.id, item as unknown as Product));
  ...
  }, [buildRequest, loading, loadingMore, page, hasMore]);
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6">
      ...
      <div className="flex-1 space-y-6">
        {/* Search Bar (Desktop) */}
        <div className="hidden md:flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-200">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <ShoppingBag className="text-indigo-600" /> Marketplace
            </h1>
            <p className="text-gray-500 text-sm">Spend your earnings on premium items</p>
          </div>
        ...
        <div className="md:hidden">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search products..."
              className="w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-600 bg-white"
              value={filters.search}
              onChange={(e) => setFilters({ ...filters, search: e.target.value })}
            />
          </div>
